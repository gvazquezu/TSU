...
public class ResponsableController {
	...
	// Crear responsable
	@PostMapping("/create")
	public ResponseEntity<?> create(@RequestBody ResponsableDto responsable) {
		Responsable responsableNew = null;
		Map<String, Object> response = new HashMap<>();
		
		try {
			responsableNew= this.responsableService.createResponsable(responsable);
			
			//Validación de campos vacios
			 if (responsable.getNombre() == null || responsable.getNombre().isEmpty()) {
	                response.put("mensaje", "El nombre del responsable es obligatorio");
	                return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
	            }
	            if (responsable.getApellido() == null || responsable.getApellido().isEmpty()) {
		...
		response.put("mensaje", "Responsable creado con éxito con el nombre" + responsableNew.getId() + ".");
	...
	public ResponseEntity<?> update(@PathVariable Long id, @RequestBody ResponsableDto responsable) {
	    ...
	    try {
            ...
            // Actualizar los campos no nulos
