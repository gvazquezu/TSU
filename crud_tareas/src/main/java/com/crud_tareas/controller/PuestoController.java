...
public class PuestoController {
	...
	//Actualizar puesto por id
	@PutMapping("/{id}")
	public ResponseEntity<?> update(@PathVariable Long id, @RequestBody PuestoDto puesto) {
		Puesto puestoActualizado = null;
	    Map<String, Object> response = new HashMap<>();

	    try {
	    	//puestoActualizado = puestoService.updatePuesto(id, puesto);
	    	puestoActualizado = puestoService.findById(id);
	    ...
	    response.put("proyecto", puestoActualizado);
	    return new ResponseEntity<Map<String, Object>>(response, HttpStatus.OK);
	}
